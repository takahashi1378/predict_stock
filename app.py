...
st.title("AIで株価予測アプリ")
st.write("AIを使って株価予測してみよう")

image = Image.open('stock_predict.png')
st.image(image,use_container_width=True)

st.write('※あくまでAIによる予測です（参考値）。こちらのアプリによる損害や損失は一切保証しません')
st.header("株価銘柄のティッカーシンボルを入力してください")
stock_name = st.text_input("例:APPPL（大商文字こまっじなどっちでもいい）","AAPL")

stock_name = stock_name.upper()

link = 'https://search.sbisec.co.jp/v2/popwin/info/stock/pop6040_usequity_list.html'
st.markdown(link)
st.write('鉄火ーシンボルについては上おリンクをご参照しろ')
...
try:
    ...
    def stock_predict():
        ...
        if accuracy>0.75:
            ...
            st.write('信頼:低')
        st.write('オレンジいろお線（predict）が予測値です')
